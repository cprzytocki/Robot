...
unsigned int ui_Arm_Motor_Speed;


unsigned long ul_Front_Motor_Position;
unsigned long ul_Back_Motor_Position;
unsigned long ui_Elevator_Motor_Position;
unsigned long ui_Grip_Motor_Position;
unsigned long ui_Arm_Motor_Position;
...
unsigned long last_encoder = 0;
unsigned long last_encoder2 = 0;

unsigned long lowest_distance = 9999;
...
//  pinMode(ci_Arm_Motor, OUTPUT);
  ...
  encoder_GripMotor.init(1.0/3.0*MOTOR_393_SPEED_ROTATIONS, MOTOR_393_TIME_DELTA);
  encoder_GripMotor.setReversed(false);  // adjust for positive count when moving forward
 
  encoder_ArmMotor.init(1.0/3.0*MOTOR_393_SPEED_ROTATIONS, MOTOR_393_TIME_DELTA);
  encoder_ArmMotor.setReversed(false);  // adjust for positive count when moving forward
...
void loop()
{
  ...
  switch(ui_Robot_State_Index)
  {
    case 0:    //Robot stopped
    {
      ...
      servo_BackMotor.writeMicroseconds(ci_Right_Motor_Stop); 
      
      servo_ElevatorMotor.write(ci_Left_Motor_Stop);
      servo_ArmMotor.write(ci_Left_Motor_Stop);
       ...
       /*************************************************************************************/
        ...
        case 2:
        {
         ...
         // Serial.println(encoder_LeftMotor.getPosition());
          ...
          if (encoder_LeftMotor.getPosition()-last_encoder >= 6)
          {
            state++;
          }
          break;
        ...
        case 3:
        {
         ...
         if (encoder_LeftMotor.getPosition()-last_encoder <=0)
         {
           state++;
         }    
          break;      
...
/*
        ...
        if (state == 2)
        {
          ...
          Serial.println(encoder_LeftMotor.getSpeed());
          Serial.println("Right Motor Speed: ");
          Serial.println(encoder_RightMotor.getSpeed());

          
        //  Serial.println(ui_Left_Motor_Speed - ui_Right_Motor_Speed);
...
void SetLeftRight()
{
  ...
  if (encoder_LeftMotor.getSpeed() < encoder_RightMotor.getSpeed())
  {
    ui_Left_Motor_Speed++;
